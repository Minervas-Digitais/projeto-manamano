...
jest.mock('../services/api', () => {
    const mockData: {
        ...
        posts: {
            ...
            '003': {
                ...
                createdAt: new Date().toISOString(),
                Comment: [
                    { id: 'c1', userId: 'user5', content: 'Comentário com erro no usuário', createdAt: new Date().toISOString() },
                    { id: 'c2', userId: 'user3', content: 'Comentário 2', createdAt: new Date().toISOString() },
                ],
            },
    ...
    const get = jest.fn((url: string) => {
        ...
        if (url.startsWith('user/')) {
            ...
            if (userId === 'user4') return Promise.reject(new Error('Erro simulado ao buscar usuário do post'));
            if (userId === 'user5') return Promise.reject(new Error('Erro simulado ao buscar usuário do comentário'));
        ...
        if (url.startsWith('archives/post/')) {
            ...
            if (postId === '001') return Promise.reject(new Error('Erro simulado ao buscar arquivos do post'));
