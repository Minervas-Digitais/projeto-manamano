    ...
    return (
        <Pressable onPress={onToggle}>
            <NotifButtonContainer>
                ...
                <ToggleThumbContainer isMoved={isActive} testID={`toggle-${text.replace(/\s+/g, '-')}`} pointerEvents="box-none">
                    {isActive ? <EllipseW width={32} testID={`EllipseW-${text.replace(/\s+/g, '-')}` } pointerEvents="box-none"/> : <EllipseB pointerEvents="box-none" width={32} testID={`EllipseB-${text.replace(/\s+/g, '-')}`} />}
                </ToggleThumbContainer>
                <ButtonImage>
                    {isActive ? <RectActv testID={`RectActv-${text.replace(/\s+/g, '-')}`} /> : <Rect testID={`Rect-${text.replace(/\s+/g, '-')}`} />}
                </ButtonImage>
            </NotifButtonContainer>
        </Pressable>
    );
